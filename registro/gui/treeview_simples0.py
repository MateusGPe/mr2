...
class TreeviewSimples:
    def __init__(
        ...
        self._ultimo_tags: Tuple[str, ...] | Literal[""] = ""
    ...
    def ativar_efeito_hover(self):
        """Ativa o efeito de destaque de linha ao passar o mouse."""

        # 1. Pega uma cor do tema atual para usar no hover
        #    Isso garante que o hover sempre combine com o tema.
        cor_hover = "#E6F7FF"
        texto_hover = "#000000"
        try:
            if style := ttkb.Style.get_instance():
                colors = cast(Colors, style.colors)
                # Pega a cor primária do tema (ex: azul no tema padrão)
                cor_base_hex = colors.primary
                # Converte a cor para RGB e a torna semi-transparente (alpha=0.25)
                # Isto cria um efeito de destaque sutil.
                r, g, b = self.view.winfo_rgb(cor_base_hex)
                # Formata a cor com alpha para uso em fundos
                cor_hover = f"#{r//256:02x}{g//256:02x}{b//256:02x}"
                # No ttkbootstrap, o ideal é usar as cores prontas, como 'active'
                cor_hover = cor_hover or colors.get("active")
                texto_hover = colors.get_foreground("primary")
        except Exception:
            pass

        # 2. Configura a tag 'hover' com a cor de fundo calculada
        self.view.tag_configure("hover", background=cor_hover, foreground=texto_hover)

        # 3. Vincula os eventos do mouse aos métodos de callback
