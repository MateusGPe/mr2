...
def start_new_session(
    ...
    if session_data["grupos"]:
        grupos_db = grupo_repo.read_filtered(
            GrupoRepository.model.nome.in_(session_data["grupos"])
        )
...
def list_all_sessions(sessao_repo: SessaoRepository) -> List[Dict]:
    ...
    return [{
        ...
        "data": s.data, "hora": s.hora, "item_servido": s.item_servido,
        "grupos": [g.nome for g in s.grupos]
    } for s in sessoes]


def get_session_details(sessao_repo: SessaoRepository, session_id: int) -> models.Sessao:
...
def get_students_for_session(
    ...
    if consumed is True:
        student_ids_to_fetch = list(consumed_student_ids.keys())
        estudantes = estudante_repo.read_filtered(estudante_repo.model.id.in_(student_ids_to_fetch))
        ...
        estudantes = estudante_repo.read_all()
    else:  # None
        estudantes = estudante_repo.read_all()
    ...
    reservas_dia = {r.estudante_id: r for r in reserva_repo.read_filtered(data=session.data, cancelada=False)}
    ...
    for est in estudantes:
        ...
        student_details.append({
            "pront": est.prontuario, "nome": est.nome,
...
def register_consumption(
    session_repo: SessaoRepository, estudante_repo: EstudanteRepository,
    reserva_repo: ReservaRepository, consumo_repo: ConsumoRepository,
    session_id: int, prontuario: str
) -> Dict[str, Any]:
    ...
    existing_consumo = consumo_repo.read_filtered(estudante_id=estudante.id, sessao_id=session_id)
    if existing_consumo:
    ...
    if session.refeicao == 'almoço':
        reserva = reserva_repo.read_filtered(estudante_id=estudante.id, data=session.data, cancelada=False)
        if reserva:
            autorizado = True
...
def update_session_groups(
    sessao_repo: SessaoRepository, grupo_repo: GrupoRepository,
    session_id: int, nomes_grupos: List[str]
):
    """Atualiza a lista de grupos autorizados para uma sessão."""
    session = get_session_details(sessao_repo, session_id)
    grupos_db = grupo_repo.read_filtered(GrupoRepository.model.nome.in_(nomes_grupos))
...
def export_session_to_xlsx(
    ...
    with xlsxwriter.Workbook(filepath) as workbook:
        ...
        for i, consumo in enumerate(consumos):
            ...
            row_data = [consumo.estudante.prontuario, session.data, consumo.estudante.nome,
                        turma, prato, consumo.hora_consumo]
    ...
    try:
        ...
        if reservas:
            ...
            if save_csv(reservas, path):
                importers_service.import_reserves_csv(estudante_repo, reserva_repo, path)
...
def sync_to_google_sheets(
    ...
    try:
        ...
        for consumo in consumos:
            ...
            rows_to_add.append([
                consumo.estudante.prontuario, session.data, consumo.estudante.nome,
                turma, prato, consumo.hora_consumo
            ])

        if rows_to_add:
            spreadsheet = google_api_service.get_spreadsheet()
            google_api_service.append_unique_rows(spreadsheet, rows_to_add,
                                                  "Consumos")  # Idealmente uma aba genérica
